...
for key, value_ in rc_in.items():
    if key in ignore_keys:
        continue
    if key not in rc_cc:
        print(f'{key} missing -> {"%s %s" % value_}')
        continue
    if rc_cc[key] != rc_in[key]:
        print(f'{key} : {"%s %s" % rc_cc[key]} -> {"%s %s" % rc_in[key]}')

if missing := [k for k in rc_cc if k not in rc_doc]:
    print()
    print("Undocumented configuration variables:")
    for m in sorted(missing):
        print("\t%s %s" % (m, "%s %s" % rc_cc[m]))

if defaults := [k for k in rc_cc if k in rc_doc and rc_cc[k] != rc_doc[k]]:
    print()
...
if stale := [k for k in rc_doc if k not in rc_cc]:
    print()
